...
<head>
    ...
    <style>
        ...
        .invoice-header {
            background: linear-gradient(90deg, #0F828C, #4DA8DA) !important;
    ...
    <div class="invoice-container">
        ...
        <main class="invoice-body">
            ...
            <div class="row mb-5">
                <div class="col-md-6">
                    ...
                    <h5 class="fw-bold mb-1">
                        {{ json_decode($booking->payment->payment_billing)->first_name . ' ' . json_decode($booking->payment->payment_billing)->last_name }}
                    </h5>
                ...
                <div class="col-md-6 text-md-end mt-4 mt-md-0">
                    ...
                    @endswitch
                    <div class="mt-3">
                        <small class="text-muted d-block">Tanggal Booking :
                            {{ \Carbon\Carbon::create($booking->bookingDeparture->booking_date)->isoFormat('DD MMMM Y') }}</small>
                        <small class="text-muted d-block">Berangkat Pukul :
                            {{ $booking->bookingDeparture->booking_time }}</small>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        <tr>
                            <td>
                                ...
                                <small class="text-muted">Dewasa :
                                    {{ $booking->bookingDeparture->num_of_people . ' - Rp.' . number_format($booking->bookingDeparture->people_price, 2, ',', '.') }}</small>
                                <br>
                                @if ($booking->bookingDeparture->num_of_adult > 0)
                                    <small class="text-muted">Anak-anak :
                                        {{ $booking->bookingDeparture->num_of_adult . ' - Rp.' . number_format($booking->bookingDeparture->adult_price, 2, ',', '.') }}</small>
                                @endif
                            </td>
                            ...
                            <td class="text-end fw-bold">Rp.
                                {{ number_format($booking->bookingDeparture->people_price * $booking->bookingDeparture->num_of_people + $booking->bookingDeparture->adult_price * $booking->bookingDeparture->num_of_adult, 2, ',', '.') }}
                            </td>
                        </tr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <small class="text-muted">Rp.
                                        {{ number_format($item->booking_extra_price, 2, ',', '.') }}</small>
                                </td>
                                <td class="text-center">1</td>
                                {{-- <td class="text-end">Rp. {{ number_format($item->booking_extra_price,2,',','.') }}</td> --}}
                                <td class="text-end fw-bold">Rp.
                                    {{ number_format($item->booking_extra_price, 2, ',', '.') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div style="font-size: 10pt">Gunakan <b>E-TIKET</b> sebagai bukti validasi untuk melakukan
                        pengecekan kepada team kami.</div>
                </div>
                <div class="col-md-4">
